  ...
  return (
    <div className="glass-card overflow-hidden border-amber-500/10">
      <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-white/5">
        ...
        <div className="p-4 space-y-4 bg-amber-500/[0.02] lg:col-span-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Market Intelligence</h3>
            <div className="px-2 py-0.5 rounded bg-amber-500/10 text-[9px] font-black text-amber-500 uppercase tracking-widest animate-pulse">Live Feed</div>
          </div>

          <div className="space-y-3">
            <div>
              <label className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1.5 block">Perpetual Contract</label>
              <div className="relative group">
                <input 
                  type="text" 
                  value={symbol}
                  onChange={(e) => setSymbol(e.target.value.toUpperCase())}
                  className="w-full bg-[#0B0E14] border border-white/10 rounded-lg px-3 py-2 text-sm font-black text-white focus:border-amber-500/50 outline-none"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[8px] font-black text-amber-500/50 uppercase">BINANCE:PERP</span>
              </div>
            </div>

            <div className="bg-[#0B0E14] rounded-lg p-3 border border-white/5 space-y-2">
               <div className="flex justify-between items-center">
                 <span className="text-[8px] text-slate-500 uppercase font-bold">Mark Price (USDT)</span>
                 <button 
                   onClick={() => setEntryPrice(parseFloat((65241.50 + (Math.random() - 0.5) * 10).toFixed(2)))}
                   className="text-[8px] font-black text-amber-500 border border-amber-500/30 px-1.5 py-0.5 rounded hover:bg-amber-500/10 active:scale-95 transition-all"
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
              <button 
                ...
                className={`group py-2 rounded-md text-[10px] font-black uppercase transition-all duration-300 relative overflow-hidden ${!isShort ? 'bg-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'bg-slate-900/50 text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}
              >
              ...
              <button 
                ...
                className={`group py-2 rounded-md text-[10px] font-black uppercase transition-all duration-300 relative overflow-hidden ${isShort ? 'bg-rose-600 text-white shadow-[0_0_20px_rgba(244,63,94,0.3)]' : 'bg-slate-900/50 text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}
              >
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-white/5 space-y-3">
          ...
          <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] font-black ${
                stats?.validity === 'VALID' ? 'bg-emerald-500/20 text-emerald-500' :
                stats?.validity === 'RISKY' ? 'bg-amber-500/20 text-amber-500' :
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest block">Risk per Trade (USD)</label>
                <div className="relative group">
                  <input 
                    type="number" 
                    value={manualRiskAmount ?? ''}
                    onChange={(e) => setManualRiskAmount(e.target.value ? parseFloat(e.target.value) : undefined)}
                    className="w-full bg-[#0B0E14] border border-white/10 group-hover:border-white/20 rounded-xl px-4 py-3 text-xl font-mono font-black text-white outline-none focus:border-amber-500/50"
              ...
              <div className="space-y-2">
                <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest block">Available Margin</label>
                <div className="relative group">
                  <input 
                    type="number" 
                    value={availableMargin || ''}
                    onChange={(e) => setAvailableMargin(parseFloat(e.target.value) || 0)}
                    className="w-full bg-[#0B0E14] border border-white/10 group-hover:border-white/20 rounded-xl px-4 py-3 text-xl font-mono font-black text-amber-500 outline-none focus:border-amber-500/50"
                    placeholder="Margin"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex bg-slate-900 rounded-md p-0.5 scale-90 origin-right">
                    {(['PRICE', 'PERCENT', 'DISTANCE'] as const).map(m => (
                      <button
                        key={m}
                        onClick={() => setSlMode(m)}
                        className={`px-1.5 py-0.5 rounded text-[8px] font-black transition-colors ${slMode === m ? 'bg-rose-500 text-white' : 'text-slate-500'}`}
                      >
                ...
                <div className="relative group">
                  <input 
                    ...
                    className="w-full bg-[#020617] border border-white/10 group-hover:border-white/20 rounded-xl px-4 py-3 text-2xl font-mono font-black text-rose-400 outline-none focus:border-rose-500/50 shadow-inner"
                    placeholder="0.00"
                  />
                </div>
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex bg-slate-900 rounded-md p-0.5 scale-90 origin-right">
                    {(['PRICE', 'PERCENT', 'DISTANCE', 'RR'] as const).map(m => (
                      <button
                        key={m}
                        onClick={() => setTpMode(m)}
                        className={`px-1.5 py-0.5 rounded text-[8px] font-black transition-colors ${tpMode === m ? 'bg-emerald-500 text-white' : 'text-slate-500'}`}
                      >
                        {m === 'PRICE' ? '$' : m === 'PERCENT' ? '%' : m}
                      </button>
                    ))}
                ...
                <div className="relative group">
                  <input 
                    ...
                    className="w-full bg-[#020617] border border-white/10 group-hover:border-white/20 rounded-xl px-4 py-3 text-2xl font-mono font-black text-emerald-400 outline-none focus:border-emerald-500/50 shadow-inner"
                    placeholder="0.00"
                  />
                </div>
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 5].map(r => (
                    <button 
                      ...
                      className="flex-1 py-1 rounded bg-emerald-500/10 hover:bg-emerald-500/20 text-[8px] font-black text-emerald-500 transition-colors"
                    >
                      1:{r}
                    </button>
                  ))}
            ...
            <div className="bg-slate-900/50 p-4 rounded-xl border border-white/5 space-y-4">
               <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-500 px-1">
                  ...
                  <span className="text-amber-500">{maxAllowedLeverage}x</span>
               </div>
               <input 
                  type="range" 
                  min="1" 
            ...
            <div className="bg-[#0B0E14] border border-white/5 rounded-xl p-4 flex items-center justify-between">
               ...
               <div className="text-right">
                  <div className="text-[9px] text-emerald-500 font-black uppercase mb-1">Expected PnL</div>
                  <div className="text-2xl font-mono font-black text-emerald-400">+${pnlAtTarget.toLocaleString()}</div>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-amber-500/[0.02] space-y-4 lg:col-span-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Execution Desk</h3>
            <span className="text-[8px] font-black text-amber-500 border border-amber-500/30 px-1.5 py-0.5 rounded uppercase">Isolated</span>
          </div>

          <div className="bg-[#020617] border border-white/10 rounded-2xl p-6 text-center shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] relative overflow-hidden group">
            ...
            <div className="absolute top-0 right-0 p-2 opacity-10 group-hover:opacity-20 transition-opacity">
               <svg className="w-12 h-12 text-amber-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12.89 3L14.85 3.4L11.11 21L9.15 20.6L12.89 3ZM19.59 12L16 8.41L17.41 7L22.41 12L17.41 17L16 15.59L19.59 12ZM4.41 12L8 15.59L6.59 17L1.59 12L6.59 7L8 8.41L4.41 12Z" /></svg>
            </div>
            <div className="text-slate-500 text-[9px] uppercase font-black tracking-[0.3em] mb-1 relative z-10">Position Size (Tokens)</div>
            ...
            <div className={`text-[10px] font-bold px-3 py-1 rounded-full inline-block uppercase mt-2 relative z-10 transition-colors ${stats?.validity === 'INVALID' ? 'bg-rose-500/20 text-rose-500' : 'bg-amber-500/10 text-amber-400'}`}>
          ...
          <div className="space-y-3">
            <div className="bg-slate-900/50 rounded-xl p-3 border border-white/5 space-y-2">
              <div className="flex justify-between items-center text-[10px] font-black uppercase">
                <span className="text-slate-500">Required Leverage</span>
                <span className={`font-mono ${stats && stats.requiredLeverage > maxAllowedLeverage ? 'text-rose-500' : 'text-amber-500'}`}>
                  {stats?.requiredLeverage.toFixed(2)}x
                </span>
              </div>
              <div className="flex justify-between items-center text-[10px] font-black uppercase">
                <span className="text-slate-500">Initial Margin</span>
                <span className="text-amber-500 font-mono">${stats?.requiredMargin.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center text-[10px] font-black uppercase pt-2 border-t border-white/5 mt-1">
                <span className="text-slate-500">Wallet Usage</span>
            ...
            {stats?.suggestions.length ? (
              <div className="bg-emerald-500/5 rounded-xl p-3 border border-emerald-500/10 space-y-2">
                <span className="text-[8px] font-black text-emerald-500 uppercase block mb-1">Optimizer Suggestions</span>
                {stats.suggestions.map((s, i) => (
                  <div key={i} className="text-[9px] font-bold text-emerald-400 leading-tight">
                    → {s}
                  </div>
                ))}
              </div>
              ...
              <div className="bg-amber-500/5 rounded-xl p-3 border border-amber-500/10">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[8px] font-black text-amber-500 uppercase">Margin Safety</span>
                  <span className="text-[8px] font-black text-emerald-400 uppercase">Secure</span>
                </div>
            ...
            <div className="flex flex-col items-center gap-1 py-1 border-t border-white/5">
               ...
               {stats && stats.validity !== 'INVALID' && (
                 <span className="text-[8px] text-emerald-500/50 font-bold uppercase italic tabular-nums">Risk Verification: {stats.units} x {stats.slDistance.toFixed(2)} = ${stats.maxRiskAmount.toFixed(2)}</span>
               )}
            </div>
          </div>
        </div>
