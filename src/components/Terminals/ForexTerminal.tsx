...
export function ForexTerminal({ capital, riskPercent }: ForexTerminalProps) {
  ...
  return (
    <div className="glass-card overflow-hidden border-rose-500/10">
      <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-white/5">
        ...
        <div className="p-4 space-y-4 bg-rose-500/[0.02] lg:col-span-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">FX Intelligence</h3>
          ...
          <div className="space-y-3">
            <div>
              <label className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1.5 block">Currency Pair</label>
              <input 
                type="text" 
                value={pair}
                onChange={(e) => setPair(e.target.value.toUpperCase())}
                className="w-full bg-[#0B0E14] border border-white/10 rounded-lg px-3 py-2 text-sm font-black text-white focus:border-rose-500/50 outline-none"
              />
            </div>

            <div className="pt-2 border-t border-white/5">
              <div className="text-[9px] text-slate-500 uppercase font-black mb-2">Contract Unit</div>
              <div className="grid grid-cols-3 gap-1">
                {(['STANDARD', 'MINI', 'MICRO'] as const).map(t => (
                  <button 
                    key={t}
                    onClick={() => setLotType(t)}
                    className={`py-1.5 rounded-md text-[9px] font-black uppercase transition-all ${lotType === t ? 'bg-rose-600 text-white shadow-lg shadow-rose-900/40' : 'bg-slate-900 text-slate-500 hover:text-slate-300'}`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <div className="bg-[#0B0E14] rounded-lg p-3 border border-white/5 space-y-2">
               <span className="block text-[8px] text-slate-500 uppercase font-bold">Pip Value (per Std Lot)</span>
               <div className="flex items-center gap-2">
                  <span className="text-sm font-mono font-black text-slate-500">$</span>
                  <input 
                    type="number" 
                    value={pipValue}
          ...
          <div className="bg-slate-900/40 rounded-lg p-3 border border-white/5 space-y-2">
             ...
             <div className="flex justify-between items-center px-1">
                <span className="text-[9px] text-slate-500 font-bold uppercase">Risk/Pip (Total)</span>
        ...
        <div className="p-5 lg:col-span-6 space-y-6">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] text-emerald-400 font-black uppercase tracking-widest block">Target (Pips)</label>
                <div className="relative group">
                  <input 
                    type="number" 
                    value={targetPips || ''}
                    onChange={(e) => setTargetPips(parseFloat(e.target.value) || 0)}
                    className="w-full bg-[#020617] border border-white/10 group-hover:border-white/20 rounded-xl px-4 py-3 text-3xl font-mono font-black text-emerald-400 outline-none focus:border-emerald-500/50 shadow-inner"
                    placeholder="0.0"
                  />
                </div>
              </div>
            ...
            <div className="bg-slate-900/50 p-4 rounded-xl border border-white/5 space-y-4">
               <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-500 px-1">
                  ...
                  <div className="flex gap-2">
                     {[100, 200, 500].map(l => (
                        <button key={l} onClick={()=>setLeverage(l)} className={`px-2 py-0.5 rounded text-[8px] font-black ${leverage === l ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-500'}`}>1:{l}</button>
                     ))}
                  </div>
               </div>
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#0B0E14] p-3 rounded-lg border border-white/5 shadow-inner">
                     <span className="block text-[8px] text-slate-500 uppercase font-black mb-1">R:R Ratio</span>
                     <span className="text-xl font-mono font-black text-white">1:{rr.toFixed(1)}</span>
                  </div>
                  <div className="bg-[#0B0E14] p-3 rounded-lg border border-white/5 shadow-inner">
                     <span className="block text-[8px] text-slate-500 uppercase font-black mb-1">Total Points</span>
            ...
            <div className="bg-[#0B0E14] border border-white/5 rounded-xl p-4 flex items-center justify-between">
               ...
               <div className="text-right">
                  <div className="text-[9px] text-emerald-500 font-black uppercase mb-1">Potential Reward</div>
                  <div className="text-2xl font-mono font-black text-emerald-400">+${(rr * (stats?.maxRiskAmount || 0)).toLocaleString()}</div>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-rose-500/[0.02] space-y-4 lg:col-span-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Institutional Desk</h3>
          ...
          <div className="bg-[#020617] border border-white/10 rounded-2xl p-6 text-center shadow-inner relative overflow-hidden group">
            ...
            <div className="text-slate-500 text-[9px] uppercase font-black tracking-[0.3em] mb-1">Lots to Execute</div>
          ...
          <div className="space-y-3">
            <div className="bg-slate-900/50 rounded-xl p-3 border border-white/5 space-y-2">
              ...
              <div className="flex justify-between items-center text-[10px] font-black uppercase pt-2 border-t border-white/5 mt-1">
                <span className="text-slate-500">Account Utility</span>
            ...
            <div className="bg-rose-500/5 rounded-xl p-3 border border-rose-500/10">
               ...
               <div className="flex justify-around items-center">
                  <div className="text-center">
                     ...
                     <div className="text-emerald-500 font-black">HIGH</div>
                  </div>
                  <div className="text-center border-l border-white/5 pl-4">
                     ...
                     <div className="text-amber-500 font-black">MED</div>
                  </div>
               </div>
            </div>
          </div>
